...
//            print($record->location->longitude . "\n"); // -93.2323
            ...
            if ($request->key_id || $request->campaign_id == 0) {
                $create['ip_address'] = $request->ip_address;
                $create['lat'] = $latitude;
...
//        exit;
        ...
        if (!empty($request)) {
            ...
            if (!empty($id)) {
                ...
                Yii::$app->mailer->compose('create_campaign', [
                        ...
                        ->setFrom(Yii::$app->params['adminEmail'])
                        ->setTo('shubhashish.das@dotsquares.com')
    ...
    public function actionUpdatecampaign() {
        ...
        if (!empty($request)) {
            ...
            $save['end_date'] = date('Y-m-d', strtotime($request->end_date));
            $save['url'] = $request->url;
            $save['no_of_keys'] = $request->no_of_keys + $request->new_keys;
            $save['key_generate_type'] = $request->key_generate_type;
...
//                            'no_of_keys' => $request->no_of_keys,
//                            'new_keys' => $extra_keys,
//                            'total_keys' => $request->no_of_keys + $extra_keys,
//                            'confirm' => Yii::$app->params['HTTP_URL'] . "/squibkeys/" . $save['slug'] . "?action=confirm",
//                            'cancel' => Yii::$app->params['HTTP_URL'] . "/squibkeys/" . $save['slug'] . "?action=cancel",
//                        ])
//                        ->setFrom(Yii::$app->params['adminEmail'])
//                        ->setTo('shubhashish.das@dotsquares.com')
//                        ->setSubject('ADDITIONAL KEYS ORDERED.')
//                        ->send();
                
                if (!empty($ids)) {
                    $user = User::find()->select(['admin_name', 'email_id', 'organization', 'work_phone'])->where(['id' => $request->user_id])->asArray()->all();
                    $emails = User::find()->select(['role', 'email_id', 'admin_name'])->where(['id' => $ids])->asArray()->all();
        ...
        if (!empty($campaigns)) {
            foreach ($campaigns as $k => $campaign) {
                ...
                $campaign_keys_count = (new yii\db\Query())
                        ...
                        ->count();
                $result[$k]['campaign_keys_count'] = $campaign_keys_count;
            }
            $status = 1;
    ...
    public function actionClear_campaign() {
        ...
        if ($request->key_type == 'all') {
            if (isset($request->name->id)) {
                $del_rows = Visitor::deleteAll(array('campaign_id' => $request->name->id));
                Yii::$app->db->createCommand()->update('campaign_keys', array('assigned' => 0), array("campaign_id" => $request->name->id))->execute();
                $save['campaign_id'] = isset($request->name->id) ? $request->name->id : 0;
                $save['keys'] = $request->key_type;
                $save['start'] = '';
                $save['end'] = '';
                Yii::$app->db->createCommand()->insert('delete_log', $save)->execute();
            } else {
                $child_ids = $this->getChilds($request->user_id);
                $child_ids = array_merge(array($request->user_id), $child_ids);
                $today = date('Y-m-d');
                $campaigns = Campaign::find()->select(['id'])->where(["user_id" => $child_ids, "status" => 1])->andWhere([ 'and', "end_date>='$today'", "start_date<='$today'"])->orderBy('name ASC')->asArray()->all();
                
                $ids = array_column($campaigns, 'id');
                $del_rows = Visitor::deleteAll(array('campaign_id' => $ids));
                Yii::$app->db->createCommand()->update('campaign_keys', array('assigned' => 0), array())->execute();
                foreach ($campaigns as $k=>$v) {
                    $save['campaign_id'] = $v['id'];
                    $save['keys'] = ucwords($request->key_type);
                    $save['start'] = '';
                    $save['end'] = '';
                    Yii::$app->db->createCommand()->insert('delete_log', $save)->execute();
                }
        ...
        } elseif ($request->key_type == 'date_range') {
            if (isset($request->name->id)) {
            ...
            if (!empty($visitors)) {
                ...
                $keys = array_column($visitors, 'key_id');
                $keys = array_unique($keys);
                $campaign_ids = array_column($visitors, 'campaign_id');
                $campaign_ids = array_unique($campaign_ids);
                
                Visitor::deleteAll(array('id' => $ids));
                
                foreach ($campaign_ids as $k=>$v) {
                    $save['campaign_id'] = $v;
                    $save['keys'] = ucwords(str_replace('_', ' ', $request->key_type));
                    $save['start'] = date('M d, Y', strtotime($request->start_date));
                    $save['end'] = date('M d, Y', strtotime($request->end_date));
                    Yii::$app->db->createCommand()->insert('delete_log', $save)->execute();
                }
                foreach ($keys as $k=>$v) {
                    $res = Visitor::find()->where(['key_id' => $v])->count();
                    if($res == 0) {
                        Yii::$app->db->createCommand()->update('campaign_keys', array('assigned' => 0), array("id" => $v))->execute();
                    }
                }
            ...
            $keys = array();
            if (isset($request->name->id)) {
                $keys = CampaignKeys::find()->select(['id'])->where(['campaign_id' => $request->name->id])->andWhere([">=", "key", $request->key_start_no])->andWhere(["<=", "key", $request->key_end_no])->asArray()->all();
            } else {
                $child_ids = $this->getChilds($request->user_id);
                $child_ids = array_merge(array($request->user_id), $child_ids);
                $today = date('Y-m-d');
                $campaigns = Campaign::find()->select(['id'])->where(["user_id" => $child_ids, "status" => 1])->andWhere([ 'and', "end_date>='$today'", "start_date<='$today'"])->orderBy('name ASC')->asArray()->all();
                $ids = array_column($campaigns, 'id');
                $keys = CampaignKeys::find()->select(['id','campaign_id'])->where(['campaign_id' => $ids])->andwhere([">=", "key", $request->key_start_no])->andWhere(["<=", "key", $request->key_end_no])->asArray()->all();
            }

            
            if (!empty($keys)) {
                $key_ids = array_column($keys, 'id');
                foreach($keys as $k=>$v) {
                    $count = Visitor::find()->where(['key_id' => $v['id']])->count();
                    if($count) {
                        $campaign_ids[$v['campaign_id']] = $v['campaign_id'];
                    }
                }
                
                $del_rows = Visitor::deleteAll(array('key_id' => $key_ids));
                if (!empty($del_rows)) {
                    foreach ($campaign_ids as $k=>$v) {
                        $save['campaign_id'] = $v;
                        $save['keys'] = ucwords(str_replace('_', ' ', $request->key_type));
                    ...
                    Yii::$app->db->createCommand()->update('campaign_keys', array('assigned' => 0), array('id' => $key_ids))->execute();
                    $status = 1;
                    $message = "Data successfully deleted";
                } else {
                    $status = 0;
                    $message = "No data exist between these key range.";
                }
            } else {
                $status = 0;
