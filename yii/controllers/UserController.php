...
class UserController extends Controller {
    ...
    public function beforeAction($event) {
        ...
        define('INSTANCEDOMAIN', 'squibdrive.com');
        return true;
    }
...
//echo json_encode(array('records' => $models), JSON_PRETTY_PRINT);
    ...
    public function actionGetusermeta() {
        ...
        if ($user) {
            $url = "http://app.squibdrive.com/profile/" . $user_id . "/";
            $title = $user->admin_name;
            $description = $user->about_me;

            $img_url = Yii::getAlias('@web') . "/uploads/users/";
            $img_path = $img_url . $user->profile_image;
            $image = $user->profile_image ? "http://app.squibdrive.com" . $img_path : "http://app.squibdrive.com/assets/img/logo-solo.png";
...
// print_r($user);exit;
    ...
    public function actionGetuser() {
        ...
        if ($user) {
            ...
            $response['profile_image'] = $user->profile_image ? $img_path : "";
            //$response['role'] = ucfirst($user->role);
            ...
            // FETCH FOLLOWERS
            $query = new Query;
            $query->from('follower')->where(['following_id' => $user_id]);
            $command = $query->createCommand();
            $followers = $command->queryAll();
            $response['followers'] = count($followers);
            
            // FETCH FOLLOWINGS
            $query = new Query;
            $query->from('follower')->where(['follower_id' => $user_id]);
            $command = $query->createCommand();
            $following = $command->queryAll();
...
//            }
    ...
    public function actionUpdateuser() {
        ...
        if ($request->url_type && $user->parent_id != 0) {
            ...
            $user->default_url = "http://" . $request->instance_name . INSTANCEDOMAIN;
        } else {
...
//return true; // dns_get_record not working
    ...
    /**
     * Create Addon Domain on WHM API.
     * @return mixed
     */
	...
	$whmusername = "root";
        ...
        $whmpassword = "LolvadJeyftajDawInCa";
...
$query = "https://72.55.146.214:2087/json-api/cpanel?cpanel_jsonapi_user=squibdrivecom&cpanel_jsonapi_apiversion=2&cpanel_jsonapi_module=AddonDomain&cpanel_jsonapi_func=addaddondomain&dir=app.squibdrive.com&newdomain=" . $vanity_domain . "&subdomain=" . $instance_name;
...
// log error if curl exec fails
    ...
    /**
     * Create Wildcard Domain on WHM API.
     * @return mixed
     */
    function create_wildcard_domain($domain) {
        ...
        $query = "https://72.55.146.214:2087/json-api/cpanel?cpanel_jsonapi_user=squibdrivecom&cpanel_jsonapi_apiversion=2&cpanel_jsonapi_module=SubDomain&cpanel_jsonapi_func=addsubdomain&dir=app.squibdrive.com&domain=*&rootdomain=" . $domain . "&disallowdot=1";
...
//        $whmpassword = "u2N+W-?VGvW.w66";
        ...
        $query = "https://72.55.146.214:2087/json-api/cpanel?cpanel_jsonapi_user=squibdrivecom&cpanel_jsonapi_apiversion=2&cpanel_jsonapi_module=AddonDomain&cpanel_jsonapi_func=listaddondomains&regex=" . $vanity_domain;
...
// log error if curl exec fails
    ...
    /**
     * Delete Addon Domain on WHM API.
     * @return mixed
     */
    function delete_addon_domain($vanity_domain, $instance_name) {
        ...
        $query = "https://72.55.146.214:2087/json-api/cpanel?cpanel_jsonapi_user=squibdrivecom&cpanel_jsonapi_apiversion=2&cpanel_jsonapi_module=AddonDomain&cpanel_jsonapi_func=deladdondomain&domain=" . $vanity_domain . "&subdomain=" . $instance_name;
...
//$_REQUEST = '{"emailid1":"arnavdots@dotsquares.com","password1":"Hello@123"}';
        ...
        if (md5($request->par1) == $request->par2) {
            ...
            if ($user->user_status == 0) {
                ...
                $user->instance_name = Yii::$app->commoncomponent->slugUnique($user->admin_name, 'user', 'instance_name');
