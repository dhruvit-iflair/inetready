app.controller('CloudCtrl', function ($scope, $http, $location, $window, $modal, $timeout, $sce, site_config) {
    $scope.$sce = $sce;
    ...
    $scope.sel_img_dwnld = function (filename, org_name)
    {
        ...
        window.location = site_config.apiUrl + "api/download?instanceid=" + instanceid + "&filepath=" + filepath + "&filename=" + filename + "&filetype=" + filetype + "&org_name=" + org_name;
    }
...
//        }
    ...
    $http({
        ...
        headers: {'Content-Type': 'application/json'}
            ...
            {
                if (response.data.status) {
                    $scope.ins_files = response.data.data;
